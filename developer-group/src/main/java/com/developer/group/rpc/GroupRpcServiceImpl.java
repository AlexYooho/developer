...
import com.developer.group.dto.SameGroupInfoResponseDTO;
...
public class GroupRpcServiceImpl implements GroupRpcService {
    ...
    @Override
    public DeveloperResult<List<SameGroupInfoResponseRpcDTO>> findSameGroupInfoList(Long targetId) {
        DeveloperResult<List<SameGroupInfoResponseDTO>> sameGroupInfoListResult = groupService.getSameGroupInfoList(targetId);
        if(!sameGroupInfoListResult.getIsSuccessful()){
            return DeveloperResult.error(SerialNoHolder.getSerialNo(), sameGroupInfoListResult.getMsg());
        }
